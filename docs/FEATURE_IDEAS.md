...
## Next: совместное редактирование вкладки по ссылке (мысли / research)

Цель: получить ссылку на открытую вкладку и отдать её человеку — он редактирует её в
реальном времени вместе со мной или просто читает. Ссылка живёт ограниченное время.

### Прямой ответ на «а можно без бэкенда, чтобы подключались прямо ко мне?»

Коротко — **на публичном интернете полностью без сервера нельзя**, и вот почему:
твоя машина почти всегда за NAT/файрволом (домашний роутер, корп-сеть), входящие
соединения к ней «снаружи» просто не проходят. Чтобы кто-то подключился «прямо ко мне»,
нужен обход NAT:

- **WebRTC (ICE)** — это самый близкий к «напрямую» вариант. Но ему всё равно нужны:
  - **signaling** — крошечный сервер, через который стороны обменяются адресами перед
    установкой P2P-канала (можно сделать на Cloudflare Worker / мелком Node — почти бесплатно);
  - **STUN** — бесплатно/дёшево (публичные есть), помогает узнать свой внешний адрес;
  - **TURN** — ретранслятор на случай «строгого» NAT (симметричный NAT, жёсткий файрвол);
    нужен примерно в 10–20% соединений, и он **платит трафиком**, т.е. это уже сервер.
  Итог: P2P «скрывает» сервер, но не убирает его — signaling обязателен, TURN нужен для
  надёжности. Если он не поднят, часть людей просто не сможет подключиться.
- **Единственный случай настоящего zero-server** — если вы в одной локальной сети (LAN):
  тогда можно связаться по локальному IP / mDNS без внешних серверов вообще. Для «скинул
  ссылку кому угодно в интернете» это не работает.

Вывод: минимально неизбежный серверный кусок — **signaling** (и на практике **TURN**).
Это в разы меньше и дешевле, чем полноценный бэкенд для хранения документа, но «нуля» нет.

### Движок совместного редактирования (общий для всех вариантов)

Не изобретать — **Yjs (CRDT) + `y-monaco` + awareness-протокол**:
- CRDT корректно сливает одновременные правки без «главного», кто решает порядок;
- `y-monaco` — готовая привязка Yjs к Monaco (у нас редактор уже Monaco);
- awareness даёт чужие курсоры/выделения/присутствие «из коробки».
Транспорт у Yjs сменный (provider): `y-websocket` (через сервер), `y-webrtc` (P2P +
signaling), `y-indexeddb` (локальное хранение). Выбор движка не зависит от выбора транспорта.

### Варианты инфраструктуры (от «мало своего сервера» к «удобно для получателя»)

1. **Serverless-релей (рекомендую для MVP).** Cloudflare Durable Objects или PartyKit
   (он поверх DO, есть готовая Yjs-интеграция). Своего сервера в эксплуатации нет, дёшево,
   глобально, сам держит комнату, TTL и проверку токена. Документ живёт в релее, пока
   комната жива → можно пережить закрытие моей вкладки. Приватность решается E2E (ниже).
2. **WebRTC P2P (`y-webrtc`).** Ближе всего к «подключаются ко мне», данные идут между
   участниками. Но нужен свой signaling и, для надёжности, TURN (см. выше). Сложнее и менее
   стабильно, зато низкая задержка и данные не оседают на релее.
3. **Локальный сервер в main + туннель.** Electron-main поднимает http+ws с мини-редактором,
   наружу — через туннель (ngrok / cloudflare tunnel / localtunnel). Ментально ровно «подключись
   к моей машине», своего постоянного бэкенда нет. Минусы: зависимость от туннель-сервиса и
   моя машина должна быть онлайн, пока идёт сессия.
4. **Managed realtime (Liveblocks / Ably / Supabase Realtime).** Минимум своей работы, но
   есть постоянная стоимость и данные проходят через вендора.

### Что важно решить помимо транспорта

- **Чем получатель открывает ссылку.** Чтобы «отправил ссылку кому угодно» — нужен
  **веб-сборка редактора** (Monaco прекрасно работает в браузере) на каком-то домене. Это
  отдельный заметный артефакт (хостинг + сборка). Альтернатива — требовать, чтобы у человека
  тоже был AuraPad и открывать `aurapad://share/...`, но это сильно сужает аудиторию.
- **Права read vs edit — обеспечивать, а не прятать кнопку.** «Читателю» мало скрыть UI:
  в CRDT он технически может слать правки. Нужно, чтобы релей/хост **отклонял** правки от
  роли reader по токену (host-authoritative модель это делает надёжнее всего).
- **Срок жизни ссылки.** Подписанный токен с `exp` + серверный TTL комнаты; плюс отзыв
  (перегенерировать ссылку), лимит участников, rate-limit.
- **Приватность / E2E (важно для позиционирования «локальный, приватный» — ср. «audio never
  leaves it»).** Любой шеринг отправляет содержимое файла с устройства — это должно быть
  осознанным действием с явным согласием. Чтобы релей видел только шифртекст: класть
  случайный ключ во **fragment ссылки** (`#key=...`) — фрагмент браузер на сервер не шлёт;
  Yjs-апдейты шифровать перед отправкой. Цена — сложнее ключи и нельзя серверных текстовых фич.
- **Жизненный цикл сессии.** Host-authoritative (P2P/туннель): документ на моей машине, закрыл
  AuraPad — сессия кончилась (это ровно «редактируем МОЮ вкладку»). Relay-persist (вариант 1):
  комната живёт без меня, другие продолжают, я переподключаюсь позже (ближе к Google Docs).

### Рекомендация и фазы

- **Движок:** Yjs + `y-monaco` + awareness — берём в любом случае.
- **Фаза 1 (MVP):** serverless-релей (Cloudflare DO / PartyKit) + E2E (ключ во fragment) +
  подписанные ссылки с TTL + роли read/edit, проверяемые на релее + минимальный веб-редактор
  для получателя.
- **Фаза 2:** WebRTC-путь (прямое соединение / ниже задержка / данные не на релее) с
  фолбэком на релей, когда P2P не встаёт.

### Оценка объёма

Большая многосоставная фича, не патч на вечер: (а) внедрить CRDT в модель вкладок/редактора,
(б) выбрать транспорт+инфру, (в) захостить веб-редактор для получателя, (г) UI шеринга
(создать ссылку, роль, TTL, список участников, чужие курсоры), (д) безопасность (E2E, токены),
(е) переподключения/офлайн-хост. Бюджетировать как отдельную большую фичу.
