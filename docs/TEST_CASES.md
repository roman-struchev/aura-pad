...
| A8 | Terminal | A shell starts in the requested directory, runs a command, returns output, and can be replaced; the toolbar opens a terminal in the panel, the panel spans the window edge to edge with the editor ending above it (not running on underneath), Cmd+F opens in-terminal search that finds text in the scrollback and Escape closes it returning focus to the shell, and Cmd+K is routed to it (git panel stays shut) while it still toggles the git panel elsewhere |
